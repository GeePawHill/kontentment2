...
class Bezier {
    ...
    fun splitToPath(fraction: Double, path: ContourBuilder) {
        if (fraction == 0.0) return
        val before = split(fraction)
        path.moveTo(before.start.x,before.start.y)
        path.curveTo(before.handle1.toVector(),before.handle2.toVector(),before.end.toVector())
    }
